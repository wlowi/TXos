...
void CalibrateSticks::run( Controls &controls) {
    ...
    for( channel_t ch = 0; ch < PORT_ANALOG_INPUT_COUNT; ch++) {
        ...
        if( calibrationStep == CALIBRATION_STEP_CENTER) {
            ...
            if( v > cfg.midPos[ch]) {
                v = ((long)(v - cfg.midPos[ch]) * (CHANNELVALUE_MAX - CHANNELVALUE_MID)) / (cfg.maxPos[ch] - cfg.midPos[ch]);
            } else if( v < cfg.midPos[ch]) {
                v = ((long)(v - cfg.midPos[ch]) * (CHANNELVALUE_MID - CHANNELVALUE_MIN)) / (cfg.midPos[ch] - cfg.minPos[ch]);
            } else {
                v = CHANNELVALUE_MID;
            }
